...
class ThreadSafeObject:
    ...
    @contextmanager
    def acquire(self, owner: str = "", msg: str = "") -> Any:
...
class CachePool:
    def __init__(self, cache_num: int = -1):
        ...
        self._cache: OrderedDict[str, ThreadSafeObject] = OrderedDict()
    ...
    ) -> Optional[Union[ThreadSafeObject, tuple]]:
        if key is None:
            return self._cache.popitem(last=False)
        else:
            return self._cache.pop(key, None)

    def acquire(self, key: str, owner: str = "", msg: str = ""):
        cache = self.get(key)
        if cache is None:
